...
- **WSL 2 as the deployment target** — the homework doesn't specify an OS. Running on Linux bare-metal or in a VM would skip Phases 0–1 entirely and avoid the WSL-specific issues (NTFS socket limitation, virtual disk sizing).
...
- **Add a liveness watchdog** — systemd `Restart=always` handles crashes but not a running-but-stuck node. A watchdog that checks block progression and restarts if stalled would catch the class of issue documented in Issue 14.
